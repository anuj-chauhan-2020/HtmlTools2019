    ...
    internal class HtmlGoToDefinition : CommandTargetBase<VSConstants.VSStd97CmdID>
    {
        ...
        private string FindFile(IEnumerable<string> extensions, out int position)
        {
            ...
            foreach (string ext in extensions)
            {
                ...
                foreach (string directory in Directory.EnumerateDirectories(global_root, "*", SearchOption.TopDirectoryOnly))//Enumerate only the top level directories in the root path of project
                {
                    if (directory != global_root + "obj" && directory != global_root + "bin")//check whether the directory is obj or bin if yes then skip them for search
                    {
                        root = directory;
